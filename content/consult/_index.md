---
title: "상담 예약"
date: 2026-08-21
type: landing
description: "전화·카카오톡·네이버 예약으로 편하게 문의하실 수 있는 해아림한의원 잠실점 온라인 상담 및 진료 예약 페이지입니다."
sections:
  - block: hero
    content:
      eyebrow: 빠른 상담 예약
      title: 온라인 상담 및 진료 예약
      text: 모든 상담은 철저한 비공개로 진행되며, 원장님이 직접 확인 후 답변드립니다.
    design:
      css_class: "bg-gradient-to-br from-primary-600 to-secondary-700 text-white"
      spacing:
        padding: ["4rem", 0, "3rem", 0]

  - block: focus-areas
    content:
      items:
        - name: 전화 상담 및 즉시 예약
          description: 진료시간 내 가장 빠른 일정 조율과 안내가 가능합니다.
          icon: hero/phone
          gradient: from-primary-400 to-primary-600
          url: "tel:02-6954-7575"
          topics: ["02-6954-7575"]
        - name: 카카오톡 1:1 비밀상담
          description: 카카오톡 채널을 통해 언제든 편안하게 문의하세요.
          icon: brands/kakaotalk
          gradient: from-amber-400 to-yellow-500
          url: "https://pf.kakao.com/_xkqufn"
          topics: ["카카오톡 1:1 비밀상담"]
        - name: 네이버 간편 진료예약
          description: 원하시는 날짜와 시간대를 선택해 대기 없이 편리하게 예약하세요.
          icon: hero/calendar-days
          gradient: from-secondary-400 to-secondary-600
          url: "https://map.naver.com/p/entry/place/1887559338?placePath=%2Fhome%3Fentry%3Dplt%26from%3Dmap%26fromPanelNum%3D1%26additionalHeight%3D76%26timestamp%3D202608220007%26locale%3Dko%26svcName%3Dmap_pcv5&searchType=place&lng=127.0819814&lat=37.5114327&c=15.00,0,0,0,dh"
          topics: ["24시간 실시간 접수"]
    design:
      layout: cards
      css_class: "bg-white dark:bg-gray-900"

  - block: faq
    content:
      title: 상담 관련 자주 묻는 질문
      items:
        - question: 다른 병·의원 약을 복용 중인데 한방 치료를 병행할 수 있나요?
          answer: |
            복용 중인 약을 임의로 중단하시는 것은 권하지 않습니다. 상담·진료 시 현재 복용 중인 약을 원장님께 알려주시면, 개인별 상태에 맞춰 병행 여부와 진행 방향을 함께 안내해 드립니다.
        - question: 첫 진료 시 시간은 얼마나 소요되나요?
          answer: |
            객관적인 검사와 1:1 심층 상담이 진행되므로 초진 시 약 50분~1시간 정도 여유 있게 내원해 주시는 것을 권장합니다.
        - question: 상담 신청 후 답변은 얼마나 걸리나요?
          answer: |
            진료시간 내 남겨주신 연락처로 최대한 빠르게 회신해 드립니다. 급하신 경우 전화 상담을 이용해 주시면 더 빠르게 안내받으실 수 있습니다.
---
