---
title: "진료시간"
date: 2026-08-21
type: landing
sections:
  - block: hero
    content:
      eyebrow: 한의원 소개
      title: 진료시간
      text: 화·금 저녁 8시까지 야간진료, 토요일·대체공휴일에도 점심시간 없이 진료합니다.
    design:
      css_class: "bg-gradient-to-br from-primary-600 to-secondary-700 text-white"
      spacing:
        padding: ["4rem", 0, "3rem", 0]

  - block: contact-info
    content:
      title: ""
      visit_title: 진료시간
      connect_title: 문의 및 예약
      office_hours:
        - "월 · 수: 10:00 ~ 19:00 (점심시간 13:00 ~ 14:00)"
        - "화 · 금 (야간진료): 10:00 ~ 20:00 (점심시간 13:00 ~ 14:00)"
        - "토요일 · 대체공휴일: 10:00 ~ 16:00 (점심시간 없이 연속 진료)"
        - "정기 휴진: 목요일 · 일요일 · 공휴일 (대체공휴일은 진료)"
      phone: "02-6954-7575"
      social:
        - icon: brands/instagram
          url: https://www.instagram.com/healimjs
      show_form: false
    design:
      spacing:
        padding: ["1rem", 0, "3rem", 0]

  - block: markdown
    content:
      text: |
        > 대체공휴일은 정상 진료하며, 목요일·일요일·공휴일은 정기 휴진입니다. 방문 전 최신 공지사항을 [공지사항 게시판](/about/notice/)에서 다시 한 번 확인해 주세요.

  - block: cta-card
    content:
      title: 진료시간에 맞춰 편하게 예약해 보세요
      text: 네이버 예약 또는 전화 상담 신청으로 원하는 시간을 미리 확인하실 수 있습니다.
      button:
        text: 네이버 예약
        url: "https://map.naver.com/p/entry/place/1887559338?placePath=%2Fhome%3Fentry%3Dplt%26from%3Dmap%26fromPanelNum%3D1%26additionalHeight%3D76%26timestamp%3D202608220007%26locale%3Dko%26svcName%3Dmap_pcv5&searchType=place&lng=127.0819814&lat=37.5114327&c=15.00,0,0,0,dh"
---
