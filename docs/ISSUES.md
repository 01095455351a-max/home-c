...
| 네이버 로그인 후 원본 후기 열람 기능 — 코드 구현 완료, 설정만 남음 | `functions/api/naver-login.js`, `functions/api/naver-callback.js`, `functions/api/reviews/[[path]].js`, `functions/_lib/session.js`, `layouts/_partials/hbx/blocks/review-grid/block.html` | R2 버킷에서 서버가 세션을 검증한 뒤에만 원본을 서빙하는 구조로 구현 완료(원본이 `public/`에 노출되지 않음). `params.yaml`의 `clinic.naver_client_id`가 비어있는 한 잠금 UI 자체가 안 보여 안전. 실사용 전 필요: (1) 네이버 개발자센터 OAuth 앱 등록 + Callback URL 등록, (2) Cloudflare R2 버킷 생성 및 원본 이미지 업로드, (3) R2 바인딩(`REVIEWS_ORIGINALS`) + 환경변수(`NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`/`SESSION_SECRET`) 등록, (4) `naver_client_id` 값 채우기. 전체 절차는 [MANUAL.md §4.11](MANUAL.md#411-자필-후기-네이버-로그인-후-원본-보기-기능-활성화) 참고 |
